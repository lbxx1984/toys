...
function init(){
	//系统初始化
	//application=new TcFrame.Application({width:1000,height:550,container:document.getElementById('container')});//嵌入到容器
	application=new TcFrame.Application({});//全屏
	if(!application.enable){return;}
	application.setStyle('backgroundColor',"#EFEFEF");
	//图标
...
function onMenuClick(event){
	switch(event.target.currentID){
		...
		case "5.2":showFunc(demoStage3D);break;
